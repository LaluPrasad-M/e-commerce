...
const password = Yup.string().required("Password is Required").min(6, "Password must be more contain at-least six characters");
const role_code = Yup.string().required("Role is Required");
const manager = Yup.mixed().default(null);
const phone = Yup.string().phone()
const dob = Yup.string().required("DOB is Required").test("dob", "Please choose a valid date of birth", (value) => {
    return moment().diff(moment(value), "years") >= 10
})

exports.register_user_schema = Yup.object().shape({
    name, email, password, role_code, manager
});
